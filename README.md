...
Abaixo, a visão lógica dos microsserviços e suas interações com o mundo externo e dispositivos de borda. O projeto utiliza o modelo C4 para descrever a separação de responsabilidades entre as camadas de apresentação, lógica de negócio e infraestrutura de dados.

### 1. Visão de Camadas (Clean View)
Uma representação isolada dos containers e componentes dentro do ecossistema SafeVision, destacando a organização modular das camadas (Presentation, Business Logic e Data & Infra) dentro do Docker Host.

![SafeVision - Camadas e Microsserviços](architecture/C4Diagram/containers2d.png)


---

### 2. Fluxo de Operação e Mensageria
Esta visão detalha a jornada completa do dado: desde a captura no **Edge** (Vision Agent), passando pelo processamento assíncrono no **Recognition Service** via RabbitMQ, até o disparo de alertas (Foto + GPS) e notificações para os usuários finais.

![SafeVision - Fluxo Completo](architecture/C4Diagram/complete1d.png)
